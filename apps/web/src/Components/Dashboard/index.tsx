  ...
  const onSubmitModal = (data: Inputs) => {
    ...
    setTimeout(() => {
      ApiClient.getInstance()
        .post({ url: "board", token: user.accessToken, data: data })
        .then((res) => {
          console.log(res);
          setModalLoading(false);
          ApiClient.getInstance()
            .get({ url: "user", token: user.accessToken })
            .then((res) => {
  ...
  return (
    <>
      <div className="flex flex-row w-full h-full flex-wrap overflow-hidden">
        <div className="flex w-full h-[8vh] bg-[#212121] border-b-2 border-[#434343]">
          <h1 className={"pl-4 pt-2 font-semibold text-[2rem] text-white"}>
            TaskFlow
          </h1>
        </div>
        <div className="flex flex-row md:flex-nowrap flex-wrap bg-[#212121]">
                ...
                return (
                  <li className="flex flex-row py-2">
                    <Link
                      ...
                      to={`/dashboard?id=${board._id}&name=${board.name}`}
                    >
                      <Folder className="mr-2" size={20} /> {board.name}
                    </Link>
                  </li>
                );
              })}
          ...
          <div className="relative flex w-[100vw] md:w-[calc(100vw-250px)] md:h-[92vh] h-[92vh]  bg-[#212121] text-white border-l-2 border-[#434343] overflow-y-scroll">
            <DashboardTasks
              name={_name ? _name : ""}
              params={_params}
              openTaskModal={() => setModalTaskOpen(true)}
              refresh={_refresh}
            />
          </div>
        </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        <ModalDialog>
          ...
          <DialogContent>Wypełnij informacje o nowym projekcie</DialogContent>
          {!modalLoading ? (
      ...
      <Modal open={modalTaskOpen} onClose={() => setModalTaskOpen(false)}>
        <ModalDialog>
          <DialogTitle>Stwórz nowy zadanie</DialogTitle>
          <DialogContent>Wypełnij informacje o nowym zadaniu</DialogContent>
          {!modalLoading ? (
            <form onSubmit={handleSubmitTask(onSubmitModalTask)}>
              <label htmlFor="name" className="mt-2">
                Nazwa
              </label>
