...
export class UsersService {
  ...
  async register(body: registerDTO): Promise<AuthUser> {
    try {
      ...
      const user_res = await this.User.findOne({ email: body.email }).populate(
        'Board',
      );
      return {
        accessToken: token,
  ...
  async getUser(user: User) {
    try {
      const _user = await this.User.findOne({ email: user.email }).populate(
        'Board',
      );
      if (!_user) {
